...
class Suggestions extends React.Component {
    ...
    return arr.map((item, i) => {
      ...
      return (
        <li
          ...
          <span className="listbox-item-description" dangerouslySetInnerHTML={markIt(item.description, this.props.query, item.markInput)} />
          <span className="listbox-item-name" dangerouslySetInnerHTML={markIt(item.nametag, this.props.query, item.markInput)} />
        </li>
      )
    })
  }
  ...
  render() {
    ...
    return (
      <div className={[this.props.classNames.suggestions, (labels.length + groups.length === 0) ? 'listbox-empty' : ''].join(" ")}>
